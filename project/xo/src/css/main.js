...
/*		$(".btn-see-more-rank").on("click",function(){
    ...
    candleCountCb:function(){
        ...
        if(Math.random()>0.5&&(!candle.lastCounts&&candle.counts>getRandom(candle.gapMin,candle.gapMax))||candle.counts-candle.lastCounts>getRandom(candle.gapMin,candle.gapMax)){
    ...
    addCandleCount:function(isRandom){
        if(!isRandom){
            ...
            Game.gameBox.candle.counts+=Math.random()*(16/3000)
        }
    },
    ...
    checkRoleAns:function(){
        ...
        //next question
            ...
            role.isAnswered=true;
            var left=role.roleList.filter(function(item){
    ...
    nextRole:function(){
        ...
        var nextNo,list;
        list=Game.gameBox.role.roleList.filter(function(item,index){
            return !item.isAnswered;
        })
    ...
    checkShowVideo:function(){
        ...
        for(var game in Game.gameBox){
            if(Game.gameBox[game].played==false){
                return;
            }
        }
    ...
    pandaFeed:function(){
        ...
        game.feedClick++;
        if(game.feedClick%5==0){
            Game.updateScore(true,50)
            if(step<game.maxFeedStep){
               Game.$feedBox.attr("data-step",step+1);
               Game.currentGame.newScore+=50;
           }
        }
    },
    //摇一摇随机得分
            ...
            loadAudio(Game.mingVideoSrc+"?"+Math.random(),function(video){
                Game.mingVideo=$(video).attr({
                    ...
                    if(Game.mingVideo[0].Paused){
                        if(!browser.versions.android){
                            Game.$videoBox.removeClass("in ")
                        }
                    }
                }).on("ended",function(){
                    Game.mingVideo[0].Paused=true;
                    Game.$videoBox.addClass("played")
                    $('.btn-go-on').show();
                })
